...
{
	detachInterrupt(TriggerPin);//PcInt::
	_status = IDLE;
}

void AsyncSonar::responseStart(AsyncSonar *item)
{
	detachInterrupt(item->TriggerPin);//PcInt::
	item->_responseStartMicros = micros();
	attachInterrupt(item->TriggerPin, responseEnd, item, FALLING);//PcInt::
}
...
void AsyncSonar::ping()
{
	...
	_startTime = millis();
	attachInterrupt(TriggerPin, responseStart, this, RISING);//PcInt::
}
